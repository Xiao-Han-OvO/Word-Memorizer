...
MainWindow::MainWindow() 
	...
	// 窗口标题
	Gtk::Label* windowTitle = Gtk::manage(new Gtk::Label("VocabMemster"));
	...
	usageMenuItem.set_label("使用说明");
	aboutMenuItem.set_label("关于");

	helpSubmenu.append(usageMenuItem);
	...
	mainBox.pack_start(*titlebarEventBox, Gtk::PACK_SHRINK);
	...
	exportWrongWordsMenuItem.set_tooltip_text("Alt+E");
	...
	usageMenuItem.signal_activate().connect(sigc::mem_fun(*this, &MainWindow::on_usage));
...
void MainWindow::on_open_file() {
	...
	if (result == Gtk::RESPONSE_OK) {
		...
		if (wordManager.loadWordsFromFile(filename)) {
			...
			errorDialog.set_decorated(false);
...
void MainWindow::on_edit_words() {
	...
	choiceDialog.set_decorated(false);
...
void MainWindow::on_export_wrong_words() {
	...
	if (wrongWords.empty()) {
		...
		infoDialog.set_decorated(false);
	...
	if (result == Gtk::RESPONSE_OK) {
		...
		if (file.is_open()) {
			...
			successDialog.set_decorated(false);
...
void MainWindow::on_review_wrong_words() {
	...
	clearButton.signal_clicked().connect([this, &wrongWordsDialog]() {
		...
		confirmDialog.set_decorated(false);
		this->apply_dialog_theme(confirmDialog);
